...
"""
Inserts set x into F[r+1], but augments x if it is necessary to ensure no two sets in F[r+1] have an intersection of rank greater than r.
"""
...
function erect_v2(n, enlargements, T=UInt16)::FullMatroid{T}
  r = 1
  E = big"2"^n-1
...
function randomized_erect_v2(n, p, T=UInt16)::FullMatroid{T}
  r = 1
  F::Vector{Set{T}} = [Set(T(0))]
  E::T = big"2"^n-1
  rank = Dict{T, UInt8}()
  ...
  while E ∉ F[r]
    ...
    if r <= length(p)
      ...
      while pr > 0 && E ∉ F[r+1]
        ...
        Acupa = rand(one_element_added)
        setdiff!(F[r+1], A)
        insert_set_v2!(Acupa, F, I, r, rank)
        pr -= 1
      end
    end
